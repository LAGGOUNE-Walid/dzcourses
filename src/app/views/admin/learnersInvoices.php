...
<html>
<head>
	<title>Admin - show all learners new invoices</title>
</head>
<style type="text/css">
		body {
		font-family: Courier;
	...
	<table>
		...
		<? foreach($invoices as $invoice): ?>
			<tr>
				...
				<td>
					<form method="POST" action="<? echo $this->url("acceptLearnerInvoice") ?>">
					<input type="hidden" name="_token" value="<? echo $token; ?>">
					<input type="hidden" name="user_id" value="<? echo $invoice->userId; ?>">
					<input type="hidden" name="id" value="<? echo (string)$invoice->_id; ?>">
					<strong><? echo $invoice->plan; ?> : </strong>
					<input type="number" name="plan" placeholder="plan">
