...
<html>
<head>
	<title>Admin - submited courses</title>
</head>
<style type="text/css">
		body {
		font-family: Courier;
  ...
  <? foreach($courses as $course): ?>
	  <tr>
	    ...
	    <td><?  foreach($videos as $video): ?>
					...
					<? endforeach; ?></td>
	    <td>
	    	<form method="POST" action="<? echo $this->url('acceptCourse'); ?>">
					<input type="hidden" name="_token" value="<? echo $token; ?>">
					<input type="hidden" name="courseId" value="<? echo (string)$course->_id; ?>">
					<input type="hidden" name="userId" value="<? echo (string)$course->user_id; ?>">
					<input type="hidden" name="courseTitle" value="<? echo (string)$course->title; ?>">
					<button type="submit">Accept</button>
				</form>
	    </td>
	    <td>
	    <form method="POST" action="<? echo $this->url('rejectCourse'); ?>">
					<input type="hidden" name="_token" value="<? echo $token; ?>">
					<input type="hidden" name="courseId" value="<? echo(string)$course->_id; ?>">
	    			...
	    			<td><textarea name="reason" style="width: 300%; height: 500px;" placeholder="Reason to deleted"></textarea>
					<br/>
