        ...
        try {
            if ($_SERVER["REQUEST_METHOD"] === "POST") { 
                if (!isset($_POST['_token'])) {
                    throw new HydrogenExceptions("[HTTP] : Invalid csrf token, please go back and refresh the page .", 8);
                }
                $request = new Request;
                if ($_SESSION['_token'] !== $request->get('_token')) {
                    throw new HydrogenExceptions("[HTTP] : Invalid csrf token, please go back and refresh the page .",9);
                }
            }
