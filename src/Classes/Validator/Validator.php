...
class Validator implements ValidatorInterface {
	...
	protected $errorsStyle = [
      "maxError"      =>  "Maximum length allowed",
      "minError"      =>  "Minimum length allowed",
      "emptyError"    =>  "Please don't send empty data",
      "notString"     =>  "We accept only alphabetic character",
      "notNum"        =>  "We accept numeric character",
      "notEmail"      =>  "Please enter a real email",
	...
	public function makeItRequired($request, $inputName) {
		(empty($request->get($inputName)) or is_null($request->get($inputName))) ? $this->errors[$inputName] = $this->errorsStyle["emptyError"] : true;
	}
	...
	$inputValue = str_replace(' ','', $inputValue);    
    	...
    	if ($type === "int") {
    		return (ctype_digit($inputValue) or is_integer($inputValue)) ? true : $this->errors[$inputName] = $this->errorsStyle["notNum"];
    	}
    	if ($type === "email") {
